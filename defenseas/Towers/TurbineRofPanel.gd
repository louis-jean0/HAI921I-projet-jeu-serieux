...
@onready var turbine_rof = preload("res://Towers/TurbineRof.tscn")
var is_holding_turbine = false
var turbine_rof_value = 100
...
func _on_gui_input(event):
	...
	if(resources_manager.money < turbine_rof_value):
		return
	var tempTurbine = turbine_rof.instantiate()
	...
	if event is InputEventMouseButton and event.button_mask == 1:
		...
		if is_position_valid(event.global_position):
			...
			tempTurbine.global_position = event.global_position
			resources_manager.remove_money(turbine_rof_value)
